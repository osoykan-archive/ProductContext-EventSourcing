...
namespace Projac.Connector.NetCore
{
    ...
    {
        /// <summary>
        ///     Resolves the <see cref="ConnectedProjectionHandler{TConnection}">handlers</see> that match the type of the message
        ///     exactly.
        /// </summary>
        ...
        public static ConnectedProjectionHandlerResolver<TConnection> WhenEqualToHandlerMessageType<TConnection>(ConnectedProjectionHandler<TConnection>[] handlers)
        {
            ...
            Dictionary<Type, ConnectedProjectionHandler<TConnection>[]> cache = handlers.GroupBy(handler => handler.Message).ToDictionary(group => group.Key, group => group.ToArray());
            return message =>
            {
                if (message == null) throw new ArgumentNullException(nameof(message));

                return cache.TryGetValue(message.GetType(), out ConnectedProjectionHandler<TConnection>[] result) ? result : new ConnectedProjectionHandler<TConnection>[0];
            };
        }

        /// <summary>
        ///     Resolves the <see cref="ConnectedProjectionHandler{TConnection}">handlers</see> to which the message instance is
        ///     assignable.
        /// </summary>
        /// <param name="handlers">The set of resolvable handlers.</param>
        /// <returns>A <see cref="ConnectedProjectionHandlerResolver{TConnection}">resolver</see>.</returns>
        public static ConnectedProjectionHandlerResolver<TConnection> WhenAssignableToHandlerMessageType<TConnection>(ConnectedProjectionHandler<TConnection>[] handlers)
        {
            ...
            return message =>
            {
                ...
                if (!cache.TryGetValue(message.GetType(), out ConnectedProjectionHandler<TConnection>[] result))
                {
