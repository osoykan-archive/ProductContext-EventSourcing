...
namespace AggregateSource
{
    ...
    public class StaticEventRouter : IConfigureStaticEventRouter
    {
        ...
        /// <param name="handler">
        ///     The state handler that should be invoked when an event of the specified type is routed to an
        ...
        ///     Thrown when <paramref name="instance" /> or <paramref name="event" /> or
        ...
        public void Route(object instance, object @event)
        {
            ...
            Dictionary<Type, Action<object, object>> handlers;
            if (!_index.TryGetValue(instance.GetType(), out handlers))
            {
                return;
            }
